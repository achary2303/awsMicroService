...
아래 화면처럼 JENKINS\_REPO\_URI가 설정된 형태로 출력되어야 합니다.
...
각 CodePipeline은 생성 시 실행되려고 할 때 실패 상태에 있지만 마이크로 서비스 1개당 CodeCommit 리포지토리는 비어 있습니다. 코드를 커밋하여 CodeCommit 리포지토리로 푸시하자마자 새 릴리스가 트리거되어 성공적으로 완료됩니다.
